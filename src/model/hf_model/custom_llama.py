...
from ..layers.layers_utils import MambaSelectiveScan


@dataclass
class CustomCausalLMOutputWithPast(CausalLMOutputWithPast):
    weights: Optional[Tuple[torch.FloatTensor]] = None
...
class CustomLlamaDecoderLayer(LlamaDecoderLayer):
    ...
    def forward(
        ...
        past_statistic: Optional[List[torch.FloatTensor]] = None,
...
class CustomLlamaModel(LlamaModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, MambaSelectiveScan) and not getattr(module.dt_proj.weight, "_no_reinit", False):
            print("first")
            dt_init_std = module.time_step_rank**-0.5
            with torch.no_grad():
                module.dt_proj.weight.uniform_(-dt_init_std, dt_init_std)

                dt = torch.exp(
                    torch.rand(module.hidden_size) * (math.log(module.dt_max) - math.log(module.dt_min))
...
class CustomLlamaForCausalLM(LlamaForCausalLM):
    ...
    def _init_weights(self, module):
        if isinstance(module, SSMLLamaFlashAttention2):
            ...
            module.query_up_proj.weight._no_reinit = True
            module.key_up_proj.weight._no_reinit = True
            module.value_up_proj.weight._no_reinit = True
